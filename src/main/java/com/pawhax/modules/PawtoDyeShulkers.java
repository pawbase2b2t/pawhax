 ...
 * Rewritten for high-ping reliability by Blake
 **/
public class PawtoDyeShulkers extends Module {
    ...
    // True if this shulker needs dyeing
    private static boolean isValidShulker(Item box, DyeColor color, boolean recolor) {
        if (isTargetShulker(box, color)) return false; // already correct colour
        if (box == Items.SHULKER_BOX) return true; // plain shulker always valid
        return recolor && isColoredShulker(box); // coloured only if recolor is on
    }
    ...
    // Scan inventory slots AND grid slots for dye, return the DyeColor with the highest total count
    ...
    private <T extends AbstractRecipeScreenHandler> void tick(T cs, int inputEnd, int invStart, int invEnd) {
        ...
        // If the active colour changed (e.g. auto-detect picked a different dye, or user changed
        // the manual setting), reset notified so we don't silently do nothing this batch
        ...
        // Move dye into grid if missing - don't wait, next tick will place the shulker
        if (dyeSlotInGrid == -1) {
            int slot = -1;
            for (int n = invStart; n < invEnd; n++) {
        ...
        // Move shulker into grid if missing - only now wait for output, both items are placed
