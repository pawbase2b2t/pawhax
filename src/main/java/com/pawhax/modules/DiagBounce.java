...
public class DiagBounce extends Module {
    ...
    public Module fromTag(NbtCompound tag) {
        super.fromTag(tag);
        freeLookEnabledByUs = tag.getBoolean("freeLookOwned").orElse(false);
        return this;
    }
    ...
    private void tickBouncing() {
        ...
        if (bouncingTicks % 20 == 0) {
            double drift = distancePointToDirection(mc.player.getEntityPos(), yawToDirection(lockedYaw), activationPos);
    ...
    private void engageObstaclePasser() {
        ...
        Vec3d travelVec = mc.player.getEntityPos().subtract(activationPos).multiply(1, 0, 1);
    ...
    private double horizontalSpeedBps() {
        ...
        return mc.player.getEntityPos().subtract(lastPos).multiply(20, 0, 20).length();
    }
